...
def get_input_ans(start_year, end_year, input_dir, n_input = 1):
    ...
    for file in trackfiles:
        ...
        lat  = np.array(df.iloc[:, 2])
        wind = np.array(df.iloc[:, 3])
...
if __name__ == "__main__": 
    ...
    if ensemble == True:
        ...
        for seed in range(30, 37):
            ...
            # 評価データの保存
            np.savez(output_dir + f'geosciAI24/predict/predict8_test{(seed):03}_alldata.npz', 
        ...
        np.savez(output_dir + 'geosciAI24/predict/predict_alldata.npz', 
        ...
        model.save(output_dir + f'/model/model_full.h5')
        np.savez(output_dir + f'geosciAI24/predict/predict_full.npz', 
                predict=predict, ans=ans_test, 
