...
def get_input_ans(start_year, end_year, input_dir, n_input = 1):
    trackfiles = []
    field = ['olr', 'qv600', 'slp', 'sst', 'u200', 'u850', 'v850']
    FIELD = ['OLR', 'QV600', 'SLP', 'SST', 'U200', 'U850', 'V850']
...
if __name__ == "__main__": 
    ...
    if ensemble == True:
        ...
        for seed in range(36, 37):
            ...
            # モデルデータの保存
            model.save(output_dir + f'/model/model8_test{(seed):03}_wo-v200.h5')
            # 評価データの保存
            np.savez(output_dir + f'geosciAI24/predict/predict8_test{(seed):03}_wo-v200.npz', 
