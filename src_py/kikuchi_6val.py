...
def cnn_model():
  ...
  model.add(Conv2D(32, (3, 3), padding='same', input_shape=(25, 144, 3*6), strides=(2,2), kernel_regularizer=l2(0.001)))   
...
def learning_curve(history, lead_time):
  ...
  plt.legend()
  plt.savefig(f'/home/maeda/machine_learning/results/kikuchi-8vals_v1/learning_curve/6vals/{(lead_time):03}day.png')
  plt.close()
...
for lead_time in lt_box:
  ...
  ipt_test  = np.concatenate([olr_ipt_test, u850_ipt_test,  u200_ipt_test, h850_ipt_test, 
  ...
  for seed in range(30):
    ...
    np.savez(f'/home/maeda/machine_learning/results/kikuchi-8vals_v1/cor/6vals/{(lead_time):03}day/seed{(seed):03}.npz', predict, y_test)
    model.save(f'/home/maeda/machine_learning/results/model/kikuchi-8vals_v1/6vals/model_{(lead_time):03}day/seed{(seed):03}.hdf5')
