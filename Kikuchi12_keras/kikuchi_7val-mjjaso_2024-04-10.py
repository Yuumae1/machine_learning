...
def preprocess(data):
  ...
  # =========
  # 訓練データの作成(通年データとする)
  idx = np.where((rt.month >= 5) & (rt.month <= 10) & (rt.year <= 2014))[0]
...
print(predict.shape)
y_test = sup_test
np.savez('/home/maeda/machine_learning/results/cnn-2d/kikuchi_7vals-mjjaso_no-lag' + str(lead_time) + 'day.npz', predict, y_test)

# モデルの保存
model.save('/home/maeda/machine_learning/results/cnn-2d/kikuchi_7vals-mjjaso_no-lag' + str(lead_time) + 'day.hdf5')
